...
dset.finrisk.pheno <- my_read_sas("nmrdata/fr2019_015_ft2019_016_nmr_10_23.sas7bdat") %>%
    ...
    dplyr::filter(VUOSI != 1992, VUOSI != 2017)
```
...
Number of participants by NMR data

```{r Number of participans by available NMR-data, echo = FALSE}
dset.finrisk.nmr %>% group_by(VUOSI) %>% summarize(n = n()) %>% mykable
```
...
dset.crosssectional.raw <- dset.finrisk.pheno %>%
    ...
    inner_join(., dset.finrisk.nmr, by = "sampleid") %>%
...
Reading NMR data for *DILGOM*. There are 
`r my_read_sas( "nmrdata/FR-NMR/nmr_fr07.sas7bdat") %>% nrow`
NMR rows for *FR07* and 
`r my_read_sas( "nmrdata/FR-NMR/nmr_dilgom14.sas7bdat") %>% nrow`
for *DILGOM*.
...
dset.dilgom <- rbind(dilgom.baseline %>% mutate(cohort = "F2007"),
    ...
    inner_join(., dset.dilgom.nmr, by = "sampleid") %>%
    select(-VUOSI, -HAVTUN) %>%
...
```{r number of pairs between HEALTH samples, echo = FALSE}
followup.pairs(dset.longitudinal.raw) %>% mykable
```
...
temp <- c("nmrdata/FR-NMR/nmr_fr97.sas7bdat",
          "nmrdata/FR-NMR/nmr_fr02.sas7bdat",
          "nmrdata/FR-NMR/nmr_fr07.sas7bdat",
          "nmrdata/FR-NMR/nmr_fr12.sas7bdat",
          "nmrdata/T20xx/T2000-nmr.sas7bdat",
...
temp[[1]] %>% colnames %>% mygrep(word = "NMR") %>% mygrep(word = "pct", complement = TRUE)
