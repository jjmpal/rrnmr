...
```{r albumin, eval = FALSE, include = FALSE}
dset <- c("FR97" = "nmrdata/FR-NMR/nmr_fr97.sas7bdat",
          "FR02" = "nmrdata/FR-NMR/nmr_fr02.sas7bdat",
          "FR07" = "nmrdata/FR-NMR/nmr_fr07.sas7bdat",
          "FR12" = "nmrdata/FR-NMR/nmr_fr12.sas7bdat",
          "FT17" = "nmrdata/FR-NMR/nmr_ft17.sas7bdat",
          "FR07" = "nmrdata/FR-NMR/nmr_fr07.sas7bdat",
          "DILGOM" = "nmrdata/FR-NMR/nmr_dilgom14.sas7bdat",
          "H2000" = "nmrdata/T20xx/T2000-nmr.sas7bdat",
          "H2011" = "nmrdata/T20xx/T2011-nmr.sas7bdat") %>%
    purrr::map(my_read_nmr) %>%
    map_df(., ~.x, .id = "cohort") %>%
    select(cohort, NMR_Albumin)

dset %>% group_by(cohort) %>% summarize(mean = mean(NMR_Albumin, na.rm = TRUE),
                                        sd = sd(NMR_Albumin, na.rm = TRUE),
                                        min = min(NMR_Albumin, na.rm = TRUE),
                                        max = max(NMR_Albumin, na.rm = TRUE),
                                        n = n(),
                                        na = sum(is.na(NMR_Albumin)))

ggplot(dset, aes(x=NMR_Albumin)) +
    facet_wrap(~cohort) +
    geom_density() +
    theme_classic()
...
```{r Metabolites}
names.manymissing <- c("nmrdata/FR-NMR/nmr_fr97.sas7bdat",
...
exclude.metabolites <- excludemetabolites(byname = c("NMR_Glycerol",
                                                     "NMR_Gln",
                                                     "NMR_DHA",
                                                     "NMR_LA",
                                                     "NMR_MUFA",
